...
class ConversationManager:
    ...
    def __init__(self, audio_manager, vision_ai=None, llm_manager=None):
        # Use provided LLM manager or create new one
        if llm_manager:
            self.llm_manager = llm_manager
            print("[Conversation]: Using shared LLM manager")
        else:
            self.llm_manager = LLMManager(audio_manager)
            print("[Conversation]: Created new LLM manager")
    ...
    def handle_greeting(self, user_input):
        ...
        # Whole-word matching only, so 'this' doesn't trigger the 'hi' greeting
        for greeting, response in greetings.items():
            if re.search(rf"\b{re.escape(greeting)}\b", user_lower):
                return response

...
class UIManager:
    def __init__(self, audio_manager, navigation_manager, voice_handler):
    ...
    def _render_destination_card(self, frame, current_time):
        ...
        frame_height, frame_width, _ = frame.shape
        dest_item = self.voice_handler.pending_destination or self.navigation_manager.current_destination
        
        if dest_item:
            dest_name = dest_item["name"]
            
            if self.voice_handler.pending_destination:
            ...
            # Draw card with modern styling
            card_x = frame_width // 4
            card_y = frame_height - 80
            card_w = frame_width // 2
            card_h = 60
            ...
            cv2.line(frame, (card_x, card_y), (card_x + card_w, card_y), card_color, 3)
            
            # Draw destination name
            cv2.putText(frame, dest_text, (card_x + 15, card_y + 25),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.55, self.colors['text_primary'], 1)
            
            # Draw status
            cv2.putText(frame, status_text, (card_x + 15, card_y + 45),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.45, card_color, 1)
            
            # Draw progress bar if available
            if progress is not None:
                bar_x = card_x + 15
                bar_y = card_y + card_h - 5
                bar_w = card_w - 30
                bar_h = 3
    ...
    def _draw_navigation_arrow(self, frame):
        ...
        if self.navigation_manager.active_route and self.navigation_manager.current_step_index < len(self.navigation_manager.active_route):
            step = self.navigation_manager.active_route[self.navigation_manager.current_step_index]
            turn = step.get('turn', '').lower()
            
            if 'left' in turn:
                cv2.arrowedLine(frame, (center_x + 50, center_y), (center_x - 50, center_y), 
                              self.colors['navigation'], 3, tipLength=0.3)
            elif 'right' in turn:
                cv2.arrowedLine(frame, (center_x - 50, center_y), (center_x + 50, center_y), 
                              self.colors['navigation'], 3, tipLength=0.3)
            else:
